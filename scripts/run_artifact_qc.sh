...
LOG="${SAMPLE_LOG_DIR}/${SAMPLE}_T2T_artifactQC_${TIMESTAMP}.log"

echo "Log file: ${LOG}"
exec > >(tee -a "${LOG}") 2>&1

echo "=================================================="
echo "  T2T artifact-aware QC start for ${SAMPLE}"
echo "  Date: $(date)"
...
########################################
# 12. STEP 10: ブラックリスト外も含めた「怪しいリード」の抽出
########################################

echo "[STEP 10] Extracting suspicious reads (low MAPQ or large soft-clip) from FINAL_BAM ..."

SUSP_SAM="${QC_DIR}/${SAMPLE}.suspicious_reads.sam"
SUSP_TOP_N=100
SUSP_NAME_COUNT="${QC_DIR}/${SAMPLE}.suspicious_readnames.top${SUSP_TOP_N}.txt"
SUSP_NAME_LIST="${QC_DIR}/${SAMPLE}.suspicious_readnames.top${SUSP_TOP_N}.list"
SUSP_TOP_SAM="${QC_DIR}/${SAMPLE}.suspicious_top${SUSP_TOP_N}.sam"
SUSP_TOP_FASTQ="${QC_DIR}/${SAMPLE}.suspicious_top${SUSP_TOP_N}.fastq"

# 10-1. FINAL_BAM 全体から「怪しいリード」を SAM で抽出
#   条件:
#     - unmapped は除外 (-F 4)
#     - MAPQ < 20 もしくは CIGAR に 20bp 以上の soft-clip (S) を含む
singularity exec --bind "${WORK_DIR}:${WORK_DIR}" "${SAMTOOLS_IMG}" \
  samtools view -h -F 4 "${FINAL_BAM}" \
  | awk '
    /^@/ {print; next}
    {
      mapq = $5;
      cigar = $6;
      soft = 0;
      # 20bp 以上の soft-clip をざっくり判定
      if (cigar ~ /([2-9][0-9]S|[1-9][0-9]{2,}S)/) {
        soft = 1;
      }
      if (mapq < 20 || soft == 1) {
        print;
...
echo "  -> suspicious SAM saved: ${SUSP_SAM}"
echo "     (criteria: MAPQ < 20 OR soft-clip >= 20bp)"

# 10-2. 怪しいリード名を出現回数順にカウントして上位 N 件を記録
awk '!/^@/ {print $1}' "${SUSP_SAM}" \
  | sort | uniq -c | sort -nr | head -n "${SUSP_TOP_N}" \
  > "${SUSP_NAME_COUNT}"

awk '{print $2}' "${SUSP_NAME_COUNT}" > "${SUSP_NAME_LIST}"

echo "  -> top${SUSP_TOP_N} suspicious read names: ${SUSP_NAME_COUNT}"

# 10-3. 上位 N の read だけを FINAL_BAM から抜き出して SAM / FASTQ 化（BLAST 用）
singularity exec --bind "${WORK_DIR}:${WORK_DIR}" "${SAMTOOLS_IMG}" \
  samtools view -h "${FINAL_BAM}" \
  | awk 'NR==FNR {name[$1]=1; next} /^@/ || ($1 in name)' "${SUSP_NAME_LIST}" - \
  > "${SUSP_TOP_SAM}"

echo "  -> suspicious top${SUSP_TOP_N} SAM: ${SUSP_TOP_SAM}"

singularity exec --bind "${WORK_DIR}:${WORK_DIR}" "${SAMTOOLS_IMG}" \
  samtools fastq -n "${SUSP_TOP_SAM}" \
  > "${SUSP_TOP_FASTQ}"

echo "  -> suspicious top${SUSP_TOP_N} FASTQ (for BLAST etc.): ${SUSP_TOP_FASTQ}"

########################################
# 13. 終了
########################################
echo "=================================================="
echo "  T2T artifact-aware QC finished for ${SAMPLE}"
echo "  End time: $(date)"
echo "  Log: ${LOG}"
echo "=================================================="